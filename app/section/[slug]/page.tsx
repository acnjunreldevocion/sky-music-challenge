...
export default async function Section({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      {['favorites-songs'].includes(slug) && <ClientFavoritesSection slug={slug} />}
      {
        !['favorites-songs'].includes(slug) && <main className="min-h-screen bg-linear-to-br from-[#fd7f00] via-[#1b1b1b] to-[#000ef5] text-white">
